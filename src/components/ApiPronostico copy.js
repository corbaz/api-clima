...
export default async function ApiPronostico(ciudad) {
    const URL_WEB = "https://api.openweathermap.org/data/2.5/forecast";
    const API_KEY = "41c24d525dac0eed946d6145c5e582c5";
    const CUANTOS_DIAS_PRONOSTICAR = 5; // 5 días como maximo de 1 a 5
    const HORAS_EN_UN_DIA = 24;
    const INTERVALO_HORASxDIA = 3;
    const CANTIDAD_INTERVALOS_PRONOSTICADOS = CUANTOS_DIAS_PRONOSTICAR * (HORAS_EN_UN_DIA / INTERVALO_HORASxDIA);
    const API_URL = `${URL_WEB}?q=${ciudad}&cnt=${CANTIDAD_INTERVALOS_PRONOSTICADOS}&appid=${API_KEY}&units=metric&lang=es`;
    ...
    if (datosJson) {
        ...
        const UTC_TimezoneOffset_Pais = datosJson.city.timezone * 1000;
        ...
        let data = datosJson.list.map(function (item, index) {
            ...
            return {
                ...
                Maxima: `${item.main.temp_max.toFixed(1)}°C ↑`,
                Minima: `${item.main.temp_min.toFixed(1)}°C ↓`,
